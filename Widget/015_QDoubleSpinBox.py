# QDoubleSpinBox 클래스는 실수를 선택, 조절하도록 하는 더블 스핀 박스 위젯을 제공
# QDoubleSpinBox
...
class MyApp(QWidget):
    ...
    def initUI(self):
        ...
        '''
            QDoubleSpinBox 객체 (self.dspinbox)를 하나 만든다.
            
            setRange() 메서드를 이용해서 선택 범위를 제한할 수 있다. 
            최소값은 0.0, 최대값은 99.99가 디폴트다.
        ...
        '''
            더블 스핀 박스 위젯의 값이 변경될 때 발생하는 시그널 (valueChanged)을 self.value_changed 
            메서드에 연결한다.
        '''
    ...
    '''
        더블 스핀 박스의 값이 변경될 때, 
        self.lbl2의 텍스트를 더블 스핀 박스의 값 (self.dspinbox.value())으로 설정
        하도록 한다. 
    '''
