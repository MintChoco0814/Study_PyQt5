# 폰트 다이얼로그는 폰트를 선택할 수 있게 해주는 다이얼로그 이다.
# https://wikidocs.net/21951
# QFontDialog
...
class MyApp(QWidget):
    ...
    def showDialog(self):
        ...
        '''
            폰트 다이얼로그를 띄우고, getFont() 메서드를 사용해서 선택한
            폰트와 불 값을 반환받는다.
            'OK' 버튼을 클릭하면 True를, 'Cancel' 버튼을 클릭하면 False를 반환한다.
        '''
        if ok:
...
if __name__ == '__main__':
    ...
    sys.exit(app.exec_())
''' 
    푸시 버튼 하나와 라벨 하나를 만들었다.
    
    QFontDialog를 사용해서, 라벨의 폰트를 바꿀 수 있다.
